...
diseases_list = {
    ...
    31: 'Osteoarthritis', 5: 'Arthritis',
    0: 'Paroxysmal Positional Vertigo', 2: 'Acne',
    38: 'Urinary tract infection', 35: 'Psoriasis', 27: 'Impetigo'
}


# Helper function: fetch recommendations for a predicted disease
def helper(disease):
    ...
    pre_list = [col for col in pre.values]

    med = medications_df[medications_df['Disease'] == disease]['Medication']
    med_list = list(med.values)

    diet = diets_df[diets_df['Disease'] == disease]['Diet']
    diet_list = list(diet.values)

    wrkout = workout_df[workout_df['disease'] == disease]['workout']
    wrkout_list = list(wrkout.values)

    return desc, pre_list, med_list, diet_list, wrkout_list
...
def get_predicted_value(patient_symptoms):
    ...
    # If no valid symptoms were found, return None to signal failure
    if not valid_symptoms:
        return None, invalid_symptoms

    predicted_index = svc.predict([input_vector])[0]
    return diseases_list[predicted_index], invalid_symptoms
...
def predict():
    if request.method == 'POST':
        ...
        # Reject empty or placeholder input
        if not symptoms_input or symptoms_input.lower() == 'symptoms':
            message = "Please enter at least one symptom."
            return render_template('index.html', message=message)

        # Parse and clean the symptom list
        user_symptoms = [s.strip().strip("[]' ").lower().replace(' ', '_')
                         for s in symptoms_input.split(',')]
        user_symptoms = [s for s in user_symptoms if s]  # drop empty strings

        predicted_disease, invalid_symptoms = get_predicted_value(user_symptoms)

        # If no valid symptoms were recognised, show a helpful error
        if predicted_disease is None:
            message = (
                f"None of the entered symptoms were recognised: "
                f"{', '.join(invalid_symptoms)}. "
                "Please check your spelling and try again."
            )
            return render_template('index.html', message=message)

        dis_des, pre_list, med_list, diet_list, wrkout_list = helper(predicted_disease)

        my_precautions = list(pre_list[0]) if pre_list else []

        # Warn the user if some symptoms were not recognised
        warning = None
        if invalid_symptoms:
            warning = (
                f"The following symptoms were not recognised and were ignored: "
                f"{', '.join(invalid_symptoms)}."
            )
